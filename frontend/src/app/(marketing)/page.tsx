...
const dashKpis = [
  { label: "Cumplimiento", value: "94%", trend: "up" },
  { label: "Certificados vigentes", value: "218", trend: "up" },
  { label: "Por vencer 30 días", value: "12", trend: "down" },
  { label: "Vencidos", value: "3", trend: "down" },
];
...
const barHeights = [30, 42, 38, 55, 48, 60, 52, 68, 62, 78, 72, 85];

function Sparkline({ up = true }: { up?: boolean }) {
  const points = up
    ? "0,14 8,10 16,12 24,7 32,9 40,4"
    : "0,4 8,7 16,6 24,10 32,9 40,13";
  return (
    <svg viewBox="0 0 40 18" width="40" height="18" className="ml-2">
      <polyline
        fill="none"
        stroke={up ? "#059669" : "#B91C1C"}
        strokeWidth="1.5"
        strokeLinecap="round"
        points={points}
      />
...
function ProductPreview() {
  return (
    <div className="relative">
      {/* Background jobsite photo layer */}
      <div className="absolute inset-0 -z-10 photo-duotone rounded-3xl opacity-40 blur-sm scale-110">
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img src={IMG.heroBackdrop} alt="" />
      </div>
      ...
      >
        <div className="bg-white rounded-2xl border border-line shadow-cardHover overflow-hidden">
          ...
          <div className="flex items-center gap-2 border-b border-line px-4 py-3 bg-canvas-2">
            ...
            <div className="flex-1 flex justify-center">
              <div className="font-mono text-xs text-ink-500 bg-white border border-line rounded-md px-3 py-1">
                app.procheck.mx/dashboard/reports
              </div>
            </div>
          </div>
          ...
          {/* Layout: sidebar + main */}
          <div className="flex">
            <div className="w-10 border-r border-line py-4 flex flex-col items-center gap-3 bg-white">
            ...
            <div className="flex-1 p-5 space-y-4 bg-white">
              ...
              <div className="grid grid-cols-4 gap-2">
                {dashKpis.map((k) => (
                  ...
                  >
                    <div className="text-[9px] text-ink-500 uppercase tracking-wide leading-tight">
                      {k.label}
                    </div>
                    <div className="flex items-center mt-1">
                      <span className="font-display text-lg font-semibold text-ink-900 leading-none">
                        {k.value}
                      </span>
                      <Sparkline up={k.trend === "up"} />
                    </div>
                  </div>
                ))}
              </div>

              {/* Cert list with avatars */}
              <div className="border border-line rounded-lg overflow-hidden">
                <div className="px-3 py-2 bg-canvas-2 border-b border-line">
                  <span className="text-[10px] uppercase tracking-wider text-ink-500 font-medium">
                    Cursos por vencer
                  </span>
                </div>
                <ul>
                  {dashCertRows.map((r, idx) => (
                    <li
                      key={r.code + r.person}
                    ...
                    >
                      <span className="flex items-center gap-2 text-ink-800 min-w-0">
                        ...
                        <img
                          src={r.avatar}
                          alt=""
                          className="h-5 w-5 rounded-full object-cover border border-line shrink-0"
                        />
